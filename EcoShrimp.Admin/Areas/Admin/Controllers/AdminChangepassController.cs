...
using EcoShrimp.Admin.Areas.Admin.ViewModels.Account;
...
{
	public class AdminChangepassController : AdminControllerBase
	{
		private readonly IDistributedCache _cache;

		public AdminChangepassController(ApplicationDbContext DbContext, IMapper mapper, IDistributedCache cache) : base(DbContext, mapper)
		{
			_cache = cache;
		}
		...
		public async Task<IActionResult> SendMail()
		{
			...
			int IdUser = int.Parse(IdClaim);
			var user = _DbContext.AppUsers.FirstOrDefault(x => x.Id == IdUser);
			if (user.Email == null)
			{
				SetErrorMesg("Vui lòng thiết lặp mail trước khi thay đổi mật khẩu!");
			...
			await _cache.SetStringAsync($"PasswordResetCode-{user.Email}", code, new DistributedCacheEntryOptions
			{
		...
		public async Task<IActionResult> VerifyCode(string code)
		{
			...
			var cachedCode = await _cache.GetStringAsync($"PasswordResetCode-{user.Email}");
		...
		public IActionResult ChangePassword(AccountVM model)
		{
			...
			user.Pass = BCrypt.Net.BCrypt.HashPassword(model.Password);
