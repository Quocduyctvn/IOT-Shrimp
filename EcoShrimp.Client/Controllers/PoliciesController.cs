...
{
	public class PoliciesController : ShrimpControllerBase
	{
		public PoliciesController(ApplicationDbContext DbContext, IMapper mapper) : base(DbContext, mapper)
		{
		}
		...
		public IActionResult Index(string keyword, int page = 1, int size = DEFAULT_PAGE_SIZE)
		{
			...
			TempData["Policies"] = _DbContext.AppPolicies.Where(x => x.Status != Status.Deleted)
											.Take(8).ToList();
